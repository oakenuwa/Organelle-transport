...
vector<vector<double> > traj2vecvec(string path, string delim, double tf, int npart, double dt)
{
    vector<vector<double> > out;
    string pos_str = "";
    vector<string> coords;
    ...
    while(getline(pos_file, pos_str))
    {
        ...
        if(count%npart==1)
        {
                t++;
                tc = t*dt;
                //cout<<"time in s: "<<tc<<endl;
        }
            if (tc < tf+dt) continue;
            else if (tc > tf+dt) break;
            else{
                //boost::trim_right(pos_str);
                boost::split(coords, pos_str, boost::is_any_of(delim));
...
double last_full_timestep(string path, int npart, double dt)
{
    string pos_str = "";
    vector<string> coords;
    ...
    while(getline(pos_file, pos_str))
    {
        ...
        //cout<<"time t: "<<t<<"\tN: "<<npart<<endl;
        //cout<<"time tprev: "<<tprev<<endl;
    }
    t = t*dt;
    tprev = tprev*dt;
    pos_file.close();
...
void write_first_tsteps(string src, int npart, double tstop, double dt)
{
 //   cout<<"does this work?"<<endl;
    ...
    ofstream write_file;
    write_file.open(src); //, ios_base::out);
