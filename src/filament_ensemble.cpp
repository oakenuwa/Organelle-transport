...
{      
    ...
    //added section to immobilize barbed ends of select filaments//
    for (int g = 0; g < 5; g++){
      //  if (f==0) cout<<"\nDEBUG: filament updates using "<<omp_get_num_threads()<<" cores";
        this->update_filament_stretching(g);
        network[g]->update_bending(t);
        network[g]->update_positions_without_barbed();
    }
