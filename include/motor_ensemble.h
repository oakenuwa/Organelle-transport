...
class motor_ensemble
{
    public:
        ...
        motor_ensemble(double mdensity, int nCargo, array<double, 2> myfov, double delta_t, double temp, double mlen, filament_ensemble* network, double v0,
        ...
        motor_ensemble(vector<vector<double> > motors, int nCargo, array<double, 2> myfov, double delta_t, double temp,
                double mlen, filament_ensemble * network, double v0, double stiffness, double max_ext_ratio,
                double ron, double roff, double rend,
                double fstall, double rcut,
                double vis, string BC);
    ...
    private:
        ...
        vector<cargo *> n_motors;
        vector<motor *> n_xlinks;
};
