...
int main(int argc, char* argv[]){
    ...
    config.add_options()
        ...
        ("num_cargo", po::value<int>(&num_cargo)->default_value(0), "number of cargos in the system")
        ...
        ("a_motor_density", po::value<double>(&a_motor_density)->default_value(0), "number of active motors / um^2")
        ...
        ("a_m_kon", po::value<double>(&a_m_kon)->default_value(10),"active motor on rate")
        //("num_cargo", po::value<int>(&num_cargo)->default_value(1),"number of cargos in the network")
        ("a_m_koff", po::value<double>(&a_m_koff)->default_value(0.1),"active motor off rate")
        ("a_m_kend", po::value<double>(&a_m_kend)->default_value(100),"active motor off rate at filament end")
        ("a_motor_length", po::value<double>(&a_motor_length)->default_value(0.1),"active motor rest length (um)")
        ...
        ("a_m_stall", po::value<double>(&a_m_stall)->default_value(0.8),"force beyond which motors don't walk (pN)")
    ...
    double nlinks = npolymer*(nmonomer-1);
    double read_time = tfinal/nframes;
    ...
    if (restart){
        ...
        double tf_prev  = min(last_full_timestep(afile,int(nmonomer*npolymer),read_time), last_full_timestep(lfile,int(nlinks),read_time));
        if (a_motor_density > 0)
            tf_prev = min(tf_prev, last_full_timestep(amfile,int(ceil(a_motor_density*xrange*yrange)),read_time));
        if (p_motor_density > 0)
            tf_prev = min(tf_prev, last_full_timestep(pmfile,int(ceil(p_motor_density*xrange*yrange)),read_time));
        ...
        actin_pos_vec   = traj2vecvec(afile, "\t ", restart_time, int(nmonomer*npolymer), read_time);
        p_motor_pos_vec = traj2vecvec(pmfile, "\t ", restart_time, int(ceil(p_motor_density*xrange*yrange)),read_time);
        a_motor_pos_vec = traj2vecvec(amfile, "\t ", restart_time, int(ceil(a_motor_density*xrange*yrange)),read_time);
        ...
        write_first_tsteps(afile, int(nmonomer*npolymer), restart_time, read_time);
        write_first_tsteps(lfile, int(nlinks), restart_time, read_time);
        write_first_tsteps(amfile, int(ceil(a_motor_density*xrange*yrange)), restart_time, read_time);
        write_first_tsteps(pmfile, int(ceil(p_motor_density*xrange*yrange)), restart_time, read_time);
    ...
    if (a_motor_pos_vec.size() == 0 && a_motor_in.size() == 0) //edited active motors to motor_ensemble for cargo motor complex
        myosins = new motor_ensemble( a_motor_density, num_cargo, {xrange, yrange}, dt, temperature,
        ...
        myosins = new motor_ensemble( a_motor_pos_vec, num_cargo, {xrange, yrange}, dt, temperature, 
                a_motor_length, net, a_motor_v, a_motor_stiffness, fene_pct, a_m_kon, a_m_koff,
    ...
    while (t <= tfinal) {
	    ...
	    if (t+dt/100 >= tinit && (count-unprinted_count)%n_bw_print==0) {
            ...
            crosslks->xlink_write(file_pm);

            //file_th << time_str<<"\tN = "<<to_string(net->get_nfilaments());
            net->write_thermo(file_th);

            file_pe << net->get_stretching_energy()<<"\t"<<net->get_bending_energy()<<"\t"<<crosslks->get_potential_energy()<<endl; //"\t"<<myosins->get_potential_energy()<<
        ...
        //update motors
        //cout<<"update motors"<<endl;
        myosins->cargo_walk(t);
